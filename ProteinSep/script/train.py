...
def train(dataloader, model, optimizer, rng, writer, epoch, opt):
    ...
    for idx, (data, _) in enumerate(tqdm(dataloader, desc="Train ProteinSepNet")):
        ...
        ##### Update ProteinSepNet #####
        optimizer.zero_grad()
        unmixed_image = model(input_image)
        
        ##### Calculate loss #####
        # l1/l2 loss
        l1_loss = L1_pixelwise(unmixed_image, target_image)
        l2_loss = L2_pixelwise(unmixed_image, target_image)
