...
def parse_arguments():
    ...
    parser.add_argument("--results_dir", type=str, default="/media/HDD1/SEPARATE/results/ProteinSep", help="root directory to save results")
    parser.add_argument("--document_dir", type=str, default="./results/ProteinSep", help="root directory to save log and namesapce")
    
    # Dataset
    parser.add_argument("--data_dir", type=str, default="/media/HDD1/SEPARATE/data", help="root directory to the data")
    parser.add_argument("--protein_list", type=str, default=['GFAP', 'MBP'], nargs="+", help="list of the protein for separation")
    ...
    try:
        ...
        datalist = glob.glob("{}/*.tif".format(opt.data_dir))
        datalist = set([data[:-8] for data in datalist])
        datalist = sorted(list(datalist))

        if len(opt.testdata_list) == 0:
            opt.testdata_list = [datalist[0]]
            
        opt.traindata_list = [data for data in datalist if data not in opt.testdata_list]
        opt.traindata_list = sorted(opt.traindata_list)
        opt.testdata_list = sorted(opt.testdata_list)
